...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim):
        ...
        self.ff_lif1 = snn.Leaky(beta = 0.95)
...
def train(model, dataloader, criterion, optimizer, scheduler, timesteps):
    running_loss = 0.0
    ...
    for data, target in tqdm(dataloader):
        ...
        output = torch.zeros((data.shape[0], 10), dtype=torch.uint8).to(device)
        model.train()

        for step in range(timesteps):
          data = spikegen.rate_conv(data)
          step_output = model(data)
          step_output = torch.div(step_output, timesteps)
          output = torch.add(output, step_output)
...
def evaluation(model, dataloader, criterion, timesteps):
    with torch.no_grad():
        test_accuracy = 0.0
        test_loss = 0.0
        for data, target in tqdm(dataloader):
...
for epoch in range(1, N_EPOCHS + 1):
    ...
    running_loss, running_accuracy = train(model, train_loader, criterion, optimizer, scheduler, timesteps=timesteps)
    print(f"Epoch : {epoch+1} - acc: {running_accuracy:.4f} - loss : {running_loss:.4f}\n")
    train_accs.append(running_accuracy)

    test_loss, test_accuracy = evaluation(model, test_loader, criterion, timesteps=timesteps)
...
plt.title("HYBRID CIFAR10\nTime Averaged Output\n" + "Train vs Test Accuracy\n" + "BATCH SIZE = " + str(BATCH_SIZE_TRAIN) + ", LR = " + str(lr) + ", PATCH SIZE = " + str(PATCH_SIZE) + ", DIM = " + str(DIM) +
          ", DEPTH = " + str(DEPTH) + ", HEADS = " + str(HEADS) + ", MLP DIM = " + str(MLP_DIM) + ", TIMESTEPS = " + str(timesteps))
plt.legend(loc='lower right')
plt.show()
torch.save(model.state_dict(), 'CIFAR10HybridTimeOutput.pth')